...
const Carousel = ({ items = [], fill = false, controls = true, priority = false, indicator = "line", translateY, aspectRatio = "original", sizes, revealedByDefault = false, thumbnail = { scaling: 1, height: "80", sizes: "120px" }, play = { auto: false, interval: 3000, controls: true }, ...rest }) => {
    ...
    return (_jsxs(Column, { fillWidth: true, fillHeight: fill, gap: "12", ...rest, aspectRatio: undefined, style: { isolation: "isolate" }, children: [items.length > 1 && play.controls && play.auto && (_jsx(Flex, { position: "absolute", top: "16", right: "16", zIndex: 1, children: _jsx(Flex, { radius: "m", background: "surface", children: _jsx(IconButton, { onClick: (e) => {
                            e.stopPropagation();
                            togglePlayPause();
                        }, variant: "secondary", icon: isPlaying ? "pause" : "play" }) }) })), _jsxs(RevealFx, { fillWidth: true, fillHeight: fill, trigger: isTransitioning, translateY: translateY, aspectRatio: aspectRatio === "original" ? undefined : aspectRatio, speed: 300, onTouchStart: (e) => {
