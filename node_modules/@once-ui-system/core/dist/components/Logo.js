...
const Logo = ({ size = "m", href, icon, wordmark, className, style, dark, light, brand, ...props }) => {
    useEffect(() => {
        if (!icon && !wordmark) {
            console.warn("Both 'icon' and 'wordmark' props are set to false. The logo will not render any content.");
        }
    }, [icon, wordmark]);
    const content = (_jsxs(_Fragment, { children: [icon && (
            // @ts-ignore
            _jsx("img", { style: {
                    height: `var(--static-space-${sizeMap[size]})`,
                    width: "auto",
                }, alt: "Trademark", src: icon })), wordmark && (
            // @ts-ignore
            _jsx("img", { style: {
                    height: `var(--static-space-${sizeMap[size]})`,
                    width: "auto",
    ...
    const renderLogo = () => {
        if (href) {
            return (_jsx(Link, { className: classNames("radius-l", "display-flex", "fit-height", dark ? "dark-flex" : "", light ? "light-flex" : "", className), style: style, href: href, "aria-label": "Trademark", ...props, children: content }));
        }
        else {
            return (_jsx(Flex, { className: classNames(className), dark: dark, light: light, radius: "l", fitHeight: true, style: style, "aria-label": "Trademark", children: content }));
        }
    };
    return enableContext ? (_jsx(ContextMenu, { dropdown: renderDropdownContent(), placement: "bottom-start", children: renderLogo() })) : (renderLogo());
};
